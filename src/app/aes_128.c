...
#define Bits19224
#define Bits25632
#define Nk 4    //4*4字节，128位密钥
#define Nr 10   //10轮加密
#define Nb 4    // AES分组大小为128位，4个字
...
void AesCipher(unsigned char* pkey,unsigned char* input, unsigned char* output)
{
 int i,round;
	memcpy(key,pkey,16);
memset(&State[0][0],0,16);                     //清零
for(i=0;i<4*Nb;i++)                       
{
 
State[i%4][i/4]=input[i];    //先写列后写行，即输入是一列一列输入的
 
}
AddRoundKey(0);//轮密钥加
 
for (round = 1; round <= (Nr - 1); round++)  // 进行10轮
{
...
unsigned char a[2000],in[16],out[16];
int len,i,x;
printf("请输入明文（16字节的整数倍）：");
//gets(a);
len=16;//strlen(a);
printf("请输入密钥（16字节）：");
//gets(key);
