...
#include "platform-specific.inc"
...
#if (uECC_WORD_SIZE == 1)
		#if uECC_SUPPORTS_secp128r1
        #define uECC_MAX_WORDS  16 /* Due to the size of curve_n. */
    #endif
		...
		#if uECC_SUPPORTS_secp128r1
        #define uECC_MAX_WORDS  4 /* Due to the size of curve_n. */
    #endif
    #if uECC_SUPPORTS_secp160r1
				#undef uECC_MAX_WORDS
		...
		#if uECC_SUPPORTS_secp128r1
        #define uECC_MAX_WORDS  2 /* Due to the size of curve_n. */
    #endif
    #if uECC_SUPPORTS_secp160r1
...
#define UECC_VLI_HW128_PC_SIMULATION  0

void uECC_vli_mult_HW128(uECC_word_t *result,
                                const uECC_word_t *left,
                                const uECC_word_t *right) {									
#if UECC_VLI_HW128_PC_SIMULATION
    uECC_vli_mult(result, left, right, 4);//temp+8,相当于结果左移0
#else    
	  uint8_t i = 0;
																	
		CP->CTRL = 0x00000002;  // (1<<1);
    ...
    // wait hw
	  while(CP->CTRL & (1 << 6))   /* operation is busy */
	  {
		    ;
	  }
...
void uECC_vli_modMult_fast(uECC_word_t *result,
                                        ...
                                        uECC_Curve curve) {
    uECC_word_t product[2 * uECC_MAX_WORDS];
    uECC_word_t temp[2 * uECC_MAX_WORDS];
    
    if (curve->num_words == 8)
    {
        /*Left = A1||A0,right = B1 || B0 ,A0 A1 B0 B1 is 128 bit
          step1: product = A0*B0
          step2: temp = A0*B1 << 128,product = product + temp
          step3: temp = A1*B0 << 128,product = product + temp
          step4: temp = A1*B1 << 256,product = product + temp
          result = product % P
        */
        
        //step1:
        uECC_vli_clear(product,16);
        uECC_vli_mult_HW128(product+8, left+4, right+4);//temp+8,相当于结果左移0
		
        //step2:
        uECC_vli_clear(temp,16);
        uECC_vli_mult_HW128(temp+4, left+4, right);//temp+4,相当于结果左移128  
        uECC_vli_add(product,product,temp,16);
        //step3:
        uECC_vli_clear(temp,16);
        uECC_vli_mult_HW128(temp+4, left, right+4);//temp+4,相当于结果左移128    
        uECC_vli_add(product,product,temp,16);
        //step4:
        uECC_vli_clear(temp,16); 
        uECC_vli_mult_HW128(temp, left, right);//temp,相当于结果左移256
        uECC_vli_add(product,product,temp,16);
    }
    else
    {
